...
const ForgetPassword: React.FC<SmsRequestPageProps> = ({ navigation, route }) => {
  ...
  const requestSmsHandler = useCallback(async () => {
    ...
    try {
      setPending(true);
      const res = await sendLoginLogic('smsRequest', { phone: payloadPhone });
      const temPassword = 'example'
      navigation.navigate('SetNewPassword', { oldPassword: temPassword });
    } catch (err) {
